                ...
                <tbody id="tableBody">
                    @foreach ($laporan as $index => $l)
                        ...
                        <tr data-status="{{ $l['status'] }}" data-verifier="{{ $verifier }}">
                            ...
                            @php
                                // use first available date key, avoid undefined index
                                $rawTanggal = $l['tanggal'] ?? $l['tanggal_pengajuan'] ?? $l['tanggal_verifikasi'] ?? ($l['created_at'] ?? null);
                            @endphp
                            <td>
                            ...
                            <td>
                                <div class="actions-cell">
                                    ...
                                    {{-- Ketua view: no PDF download link --}}
                                </div>
                            </td>
                        </tr>
                    @endforeach
    ...
    <script>
        const detailData = [
            @foreach ($laporan as $index => $l)
            {
                ...
                tanggal_verifikasi: "{{ $l['status'] == 'Diverifikasi' || $l['status'] == 'Ditolak' ? date('d/m/Y H:i', strtotime(($l['tanggal_verifikasi'] ?? $l['tanggal'] ?? date('Y-m-d')) . ' +2 days')) : '' }}",
                angsuran: [
                    @php
                        $jumlah = $l['jumlah'];
                        $tenor = rand(12, 36);
